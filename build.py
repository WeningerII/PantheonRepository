...
def main() -> None:
    ...
    # Two forms of the same UI code. The artifact inlines it (it opens over
    # file:// with no siblings); the Pages shell links it as one hashed file.
    #
    # Concatenating the 13 sources is semantics-preserving, not a bundling
    # step: each is a classic <script>, so their top-level const/let already
    # share ONE global lexical environment (this is exactly why toplevel
    # mangling breaks them — see docs/load-performance-findings.md §6). The
    # only behavioural difference is that a top-level throw now stops the
    # sources after it instead of just its own block, and an app that throws
...
<style>
  ...
  #boot .lead {
    ...
    color: #3A3734;   /* 9.2:1 on the #FAFAF7 boot background */
    max-width: 34rem;
    text-align: center;
    text-wrap: pretty;
    margin-top: -6px;
...
<div id="boot" role="status" aria-live="polite">
  ...
  <!-- Static, and deliberately the largest text on the boot screen. The app is
       client-rendered, so until React mounts the biggest contentful element on
       the page was a 279x19 search-input placeholder — which made Lighthouse
       time LCP at "React finished booting" (8.0 s in the field). This paragraph
       paints with the document instead. It is the site's own canonical
       description (the same sentence as the og:description meta), so it cannot
       drift from what the page claims to be, and it tells a waiting visitor
       what they are loading rather than showing them a bare spinner.
       It lives in the position:fixed #boot overlay, out of flow, so removing it
       on ready cannot shift layout — CLS stays 0. -->
  <div class="lead">A source-cited index of the world's mythological and historical figures — genealogies, domains, epithets, iconography, and cult — across hundreds of traditions.</div>
...
"""
    ...
    if pages:
        # The UI code ships as ONE hashed external file rather than ~122 KB gz
        # of inline <script>. That is not about bytes — it is the same bytes
        # either way — but about where they sit. Lighthouse's Lantern has no
        # streaming model, so nothing paints until the WHOLE document lands;
        # inlining put the entire app on the first-paint critical path. Moving
        # it out took the document 149 KB gz -> ~27 KB gz and measured mobile
        # FCP 2,229 ms -> 1,335 ms. It also means the app survives a shell
        # rebuild in the browser cache, which an inline copy never could.
        #
        # Hashed, like every other pinned asset — Pages fixes max-age=600, so
        # the filename is the only cache-bust available. It sits beside the
        # shell in dist/site rather than in dist/data: data/ is generated by
        # build-tiers.cjs under a byte-identical-regeneration contract
        # (test/tiers.test.cjs), and a file this build writes has no business
        # in that tree. The src stays page-relative so the shell still serves
        # from a project sub-path.
        ui_hash = hashlib.sha256(ui_bundle_body.encode('utf-8')).hexdigest()[:12]
        ui_name = f'app-{ui_hash}.js'
        (DIST / 'site').mkdir(exist_ok=True)
        for stale in (DIST / 'site').glob('app-*.js'):
            if stale.name != ui_name:
                stale.unlink()
        (DIST / 'site' / ui_name).write_text(ui_bundle_body, encoding='utf-8')
        # defer, or the parser stalls on it and it is render-blocking all over
        # again (measured: 1,370 ms, and FCP regressed 1,606 -> 2,262 ms without
        # it). Order is preserved exactly: deferred scripts run after parsing in
        # DOCUMENT order, so react/react-dom (head) still precede this, and the
        # non-deferred inline data layer — __PR_DATA, core.js, pr-boot — has
        # already run by then. pr-boot only ever reaches the UI through events
        # fired off async tier fetches, which cannot land before DOMContentLoaded.
        script_blocks = f'<script defer src="{ui_name}"></script>'
    else:
        script_blocks = script_blocks_inline
