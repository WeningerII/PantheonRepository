...
| S02 | Browse | the full corpus renders as table rows | ✅ PASS | 2001 figure rows |
| S03 | Search / filter | typing in search narrows the table; clearing restores it | ✅ PASS | full=2001, "thor"→6, restored=2001 |
...
| S12 | Graph view | the default graph mounts a populated node-link diagram | ✅ PASS | 608 graph nodes |
...
| S18 | Items registry | the Items view lists the object corpus grouped by kind | ✅ PASS | 1358 item rows, grouped |
...
| S22 | Persistence — corruption | corrupted localStorage JSON falls back to the seed | ✅ PASS | fell back to seed (2001 rows), no errors |
...
| S30 | Browse — tradition filter | selecting a tradition in the rail narrows the table | ✅ PASS | Norse filter: 2001 → 41 → 2001 |
