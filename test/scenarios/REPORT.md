...
| S02 | Browse | the full corpus renders as table rows | ✅ PASS | 4147 figure rows |
| S03 | Search / filter | typing in search narrows the table; clearing restores it | ✅ PASS | full=4147, "thor"→9, restored=4147 |
...
| S12 | Graph view | the default graph mounts a populated node-link diagram | ✅ PASS | 800 graph nodes |
| S13 | Graph focus | deep-linking #/graph/<id> focuses that figure | ✅ PASS | focused "Heracles" with neighbor list |
| S14 | Graph path-finding | path mode finds a shortest relation path between two figures | ✅ PASS | path found: "Path · 1 hopclearHeraclesDemigod · Greek↓parentZeusDeity · G" |
| S15 | Atlas view | the atlas mounts all mapped territories | ✅ PASS | 344 paths, 247 traditions |
...
| S18 | Items registry | the Items view lists the object corpus grouped by kind | ✅ PASS | 2762 item rows, grouped |
| S19 | Item custody | an item detail names its custody chain and links registry holders | ✅ PASS | custody chain with linked holder |
| S20 | Keyboard nav | j/k move the cursor, Enter opens it, Escape closes | ✅ PASS | cursor "Aatxe" opened + closed |
...
| S22 | Persistence — corruption | corrupted localStorage JSON falls back to the seed | ✅ PASS | fell back to seed (4147 rows), no errors |
...
| S29 | Browse — sort | sorting by tradition reorders the table | ✅ PASS | reordered from alpha "Aan Alakhchyn Khotun" |
| S30 | Browse — tradition filter | selecting a tradition in the rail narrows the table | ✅ PASS | Norse filter: 4147 → 67 → 4147 |
...
| S34 | Detail — figure nav | Prev/Next step through the filtered list and return | ✅ PASS | Prev/Next: "Aan Alakhchyn Khotun" ↔ "Aatxe" |
