...
| S02 | Browse | the full corpus renders as table rows | ✅ PASS | 6786 figure rows |
| S03 | Search / filter | typing in search narrows the table; clearing restores it | ✅ PASS | full=6786, "thor"→18, restored=6786 |
...
| S15 | Atlas view | the atlas mounts all mapped territories | ✅ PASS | 714 paths, 566 traditions |
...
| S18 | Items registry | the Items view lists the object corpus grouped by kind | ✅ PASS | 3208 item rows, grouped |
| S19 | Item custody | an item detail names its custody chain and links registry holders | ✅ PASS | custody chain with linked holder |
| S20 | Keyboard nav | j/k move the cursor, Enter opens it, Escape closes | ✅ PASS | cursor "A'amo" opened + closed |
...
| S22 | Persistence — corruption | corrupted localStorage JSON falls back to the seed | ✅ PASS | fell back to seed (6786 rows), no errors |
...
| S29 | Browse — sort | sorting by tradition reorders the table | ✅ PASS | reordered from alpha "A'aisa" |
| S30 | Browse — tradition filter | selecting a tradition in the rail narrows the table | ✅ PASS | Norse filter: 6786 → 101 → 6786 |
...
| S34 | Detail — figure nav | Prev/Next step through the filtered list and return | ✅ PASS | Prev/Next: "A'aisa" ↔ "A'amo" |
