...
| S02 | Browse | the full corpus renders as table rows | ✅ PASS | 2109 figure rows |
| S03 | Search / filter | typing in search narrows the table; clearing restores it | ✅ PASS | full=2109, "thor"→6, restored=2109 |
...
| S18 | Items registry | the Items view lists the object corpus grouped by kind | ✅ PASS | 1471 item rows, grouped |
| S19 | Item custody | an item detail names its custody chain and links registry holders | ✅ PASS | custody chain with linked holder |
| S20 | Keyboard nav | j/k move the cursor, Enter opens it, Escape closes | ✅ PASS | cursor "'Oro" opened + closed |
...
| S22 | Persistence — corruption | corrupted localStorage JSON falls back to the seed | ✅ PASS | fell back to seed (2109 rows), no errors |
...
| S30 | Browse — tradition filter | selecting a tradition in the rail narrows the table | ✅ PASS | Norse filter: 2109 → 41 → 2109 |
...
| S34 | Detail — figure nav | Prev/Next step through the filtered list and return | ✅ PASS | Prev/Next: "'Antara ibn Shaddad" ↔ "'Oro" |
