...
| S02 | Browse | the full corpus renders as table rows | ✅ PASS | 2198 figure rows |
| S03 | Search / filter | typing in search narrows the table; clearing restores it | ✅ PASS | full=2198, "thor"→6, restored=2198 |
...
| S12 | Graph view | the default graph mounts a populated node-link diagram | ✅ PASS | 696 graph nodes |
...
| S18 | Items registry | the Items view lists the object corpus grouped by kind | ✅ PASS | 1536 item rows, grouped |
| S19 | Item custody | an item detail names its custody chain and links registry holders | ✅ PASS | custody chain with linked holder |
| S20 | Keyboard nav | j/k move the cursor, Enter opens it, Escape closes | ✅ PASS | cursor "'Antara ibn Shaddad" opened + closed |
...
| S22 | Persistence — corruption | corrupted localStorage JSON falls back to the seed | ✅ PASS | fell back to seed (2198 rows), no errors |
...
| S29 | Browse — sort | sorting by tradition reorders the table | ✅ PASS | reordered from alpha "'Aho'eitu" |
| S30 | Browse — tradition filter | selecting a tradition in the rail narrows the table | ✅ PASS | Norse filter: 2198 → 41 → 2198 |
...
| S34 | Detail — figure nav | Prev/Next step through the filtered list and return | ✅ PASS | Prev/Next: "'Aho'eitu" ↔ "'Antara ibn Shaddad" |
