...
| S02 | Browse | the full corpus renders as table rows | ✅ PASS | 1964 figure rows |
| S03 | Search / filter | typing in search narrows the table; clearing restores it | ✅ PASS | full=1964, "thor"→6, restored=1964 |
...
| S12 | Graph view | the default graph mounts a populated node-link diagram | ✅ PASS | 604 graph nodes |
...
| S18 | Items registry | the Items view lists the object corpus grouped by kind | ✅ PASS | 1345 item rows, grouped |
...
| S22 | Persistence — corruption | corrupted localStorage JSON falls back to the seed | ✅ PASS | fell back to seed (1964 rows), no errors |
...
| S30 | Browse — tradition filter | selecting a tradition in the rail narrows the table | ✅ PASS | Norse filter: 1964 → 41 → 1964 |
